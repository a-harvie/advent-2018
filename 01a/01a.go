...
func main() {
	...
	for i := 0; i < len(tests); i++ {
		fmt.Println(tests[i])
		f = getRepeatFreq(start, tests[i])
		fmt.Printf("Test %v : %v %v passed: %v\n", i, f, testResults[i], f == testResults[i])
	}

	f = getRepeatFreq(start, input)
	fmt.Printf("Answer: %v", f)
}

func getFreq(start int, deltas []int) int {
